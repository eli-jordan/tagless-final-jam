...
object domain {
    ...
    /**
     * Here we define the set of operations (or algebras) that we can perform on the data types.
     *
     * UserRepository - allows users to be saved and retrieved
     * BookRepository - allows books to be saved and retrieved
     *
     * ReadingListService - defines the operations we need to perform on reading lists.
     *
     * Notice that each of these traits have a similar structure.
     *
     * 1. They each take a higher-kinded type argument that represents the effect type the operations will be run within.
     *    Effect types are things like Future, Task, IO, Id etc.
     *
     * 2. Rather than simply returning a value e.g. "getUser(id: UserId): User"
     *    they return a value wrapped in an effect type e.g. "getUser(id: UserId): F[User]"
     *    to indicate that there is some kind of effect required to perform the operation.
     *    For example to get a user by id, we would generally go to a database or other
     *    persistent storage, this fact is captured by the effect.
     */
